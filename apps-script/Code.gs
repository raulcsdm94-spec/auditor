...
var HEADERS = [
  ...
  "Email",                   // 7  G
  "Estratégia",              // 8  H
  "Assunto (cold-call)",     // 9  I
  "Email cold-call (texto)", // 10 J
  "Pronto a enviar",         // 11 K
  "Email enviado",           // 12 L
  "Data envio",              // 13 M
  "Relatório cliente",       // 14 N
  "Relatório completo",      // 15 O
  "Screenshot",              // 16 P
  "Notas",                   // 17 Q
];
...
function getSheet_() {
  ...
  if (sheet.getLastRow() === 0) {
    ...
    sheet.setColumnWidth(10, 420); // "Email (texto)" mais larga para rever
  }
  return sheet;
}
...
function upsertRow_(sheet, data, links) {
  ...
  set(2, key);
  set(3, data.estado);
  if (isNum_(data.criticos)) atual[3] = data.criticos;
  if (isNum_(data.altos)) atual[4] = data.altos;
  if (isNum_(data.medios)) atual[5] = data.medios;
  set(7, data.email);
  set(8, data.estrategia);
  set(9, data.assunto);
  set(10, data.emailTexto);
  set(11, data.prontoEnviar);
  set(12, data.emailEnviado);
  set(13, data.dataEnvio);
  setLink(14, links.cliente, "Abrir PDF");
  setLink(15, links.completo, "Abrir");
  setLink(16, links.screenshot, "Ver");
  set(17, data.notas);
